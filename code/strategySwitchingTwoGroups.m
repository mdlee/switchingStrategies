...
doPrint = false;                      % set to true to print eps and png figures to figures subfolder
...
bfCutoff = 0.01;                        % small value around 0 and 1 for Bayes factor test of latent-mixture
CIbounds = [2.5 97.5];                  % bounds on credible intervals
...
for analysisIdx = 1:nAnalyses
    ...
    switch modelName
        ...
        case 'ssLM_TTBEQWTTBWADDGuess'
            data.theta = d.choice(subjectList, :, :);
    end
    
    fileName = sprintf('%s_%s_A.mat', modelName, dataName);
    if exist(sprintf('storage/%s', fileName), 'file')
        ...
        save(sprintf('storage/%s', fileName), 'stats', 'chains', 'diagnostics', 'info');
    end
    ...
    if bfLow >0 | bfHigh >0
        ...
        data.x = ones(nSubjects, 1);
    end
    
    fileName = sprintf('%s_%s_B.mat', modelName, dataName);
    if exist(sprintf('storage/%s', fileName), 'file')
        load(sprintf('storage/%s', fileName), 'chains');
    else
        tic;
    ...
    fprintf('Modal group gammas are %1.2f [%1.2f, %1.2f] and %1.2f [%1.2f, %1.2f]\n', muGamma(1), prctile(chains.muGamma_1(:), [2.5 97.5]), muGamma(2), prctile(chains.muGamma_2(:), [2.5 97.5]));
    fprintf('Standard deviations are %1.2f and %1.2f\n', sigmaGamma);
    ...
    for posteriorIdx = 1:max(nPosteriors)
        ...
        fileName = sprintf('%s_%s_C_posterior%d.mat', modelName, dataName, posteriorIdx);
        if exist(sprintf('storage/%s', fileName), 'file')
            load(sprintf('storage/%s', fileName), 'chains');
        else
            tic; % start clock
    ...
    switch modelName
        ...
        otherwise
            % some early save files used the name "choice" instead of "theta"
            warning off
            try
                theta = reshape(codatable(chains, 'theta', @mean), length(subjectList), d.nTrials, nStrategies);
            catch
    ...
    for idx = 1:nStrategies
        fprintf('base-rate for starting strategy %s: %1.2f (%1.2f, %1.2f)\n', strategyList{idx}, piPrime(idx), prctile(chains.(sprintf('piPrime_%d', idx))(:), CIbounds));
    end
    
    % write to file latex table of transition probabilities
    if printTable
        outputTransitionTable(dataName, modelName, strategyList, chains, CIflag, CIbounds);
    end
    ...
    if drawIndividuals
        ...
        for figIdx = 1:ceil(length(subjectList)/(nRows*nCols))
            ...
            if doPrint
                ...
                print(sprintf('figures/LMindividuals_%s.png', dataName), '-dpng');
                print(sprintf('figures/LMindividuals_%s.eps', dataName), '-depsc');
            end
    ...
    if drawChange
        ...
        for figIdx = 1:ceil(length(subjectListChange)/(nRows*nCols))
            ...
            if doPrint
                ...
                print(sprintf('figures/LMindividualsChange_%s.png', dataName), '-dpng');
                print(sprintf('figures/LMindividualsChange_%s.eps', dataName), '-depsc');
            end
    ...
    if drawOverall
        ...
        if doPrint
            ...
            print(sprintf('figures/LMall_%s.png', dataName), '-dpng');
            print(sprintf('figures/LMall_%s.eps', dataName), '-depsc');
        end
...
% this is bespoke for the Brusovansky analysis
if printCombinedTable
