%% strategy switching
% v2 considers full posterior of switch point possibilities
...
analysisList = {...
   ...
   %    {'L%tAl2014Exp1', 'ss_TTBTallyWADDGuess', 1:30, [4, 2]}; ...
   };
...
for analysisIdx = 1:nAnalyses
   ...
   fprintf('Modal group gamma is %1.2f (95CI = [%1.2f, %1.2f], with mean standard deviation %1.2f\n', muGamma, prctile(chains.muGamma(:), [2.5 97.5]), sigmaGamma);
   ...
   %% Inference given switch points, with a full subject run for each possible posterior
   ...
   for posteriorIdx = 1:max(nPosteriors)
      ...
      if exist(['storage/' modelName '_' dataName  '_B_posterior' int2str(posteriorIdx) '.mat'], 'file')
         load(['storage/' modelName '_' dataName '_B_posterior' int2str(posteriorIdx)'], 'chains');
      else
         tic; % start clock
         [stats, chains, diagnostics, info] = callbayes(engine, ...
         ...
         save(['storage/' modelName '_' dataName '_B_posterior' int2str(posteriorIdx)'], 'stats', 'chains', 'diagnostics', 'info');
      end
      
   end
   
   % work with mode for major plots
   load(['storage/' modelName '_' dataName '_B_posterior1'], 'chains');
